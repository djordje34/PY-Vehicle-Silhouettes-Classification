...
def Setup():
    ...
    tuner.search(X_train, y_train, epochs=100,validation_data=(X_val, y_val), callbacks=[stop_early],batch_size=64)
    all_hps = tuner.get_best_hyperparameters(num_trials=5)
    best_hp=all_hps[0]

    h_model = tuner.hypermodel.build(best_hp)
    history=h_model.fit(X_train, y_train, epochs=100, validation_data=(X_val, y_val),batch_size=64)
    Plotter.accuracy(history)
    h_model.summary()
    ...
    for hp in all_hps[1:]:
        if "tuner/trial_id" in hp.values:
            ...
            scores=pd.concat([scores, pd.DataFrame([hp.values])], ignore_index=True)
